...
//fixing the c01 problem 
replace c01= c01/1000
//(6,031 real changes made)

//to create variable for all other state revenues 
generate other_state_rev=tot_state_rev-c01


****
//descriptive tables
//code for creating separate tables for 2002-14 and 2008-14 
//keep it as rough work - DO NOT SAVE!!
keep if sample==1 

//to create separate descriptive file for 2008-14
keep if year>2007
...
*descriptive statistics for data cleaning

sum tlocrev tot_state_rev c01 other_state_rev tot_fed_rev stu_teach_ratio per_white per_black per_hisp n_tot_race per_frl per_iep per_lep  
sum BA LessHS MedianIncome Poverty Population White Black Hispanic Asian Under18 Over65

//run the code from the above segement - descriptive statistics 

//changes in the dataset fr4om negative to missing values 
//1.For tlocrev, c01, and other_state, replace all negative values with a missing value (.)
 
replace tlocrev=. if tlocrev<0
replace c01=. if c01<0
replace other_state_rev=. if other_state_rev<0
//(5,117 real changes made, 5,117 to missing)

edit if tot_fed_rev==0
/* 6 observations, all observations are in school districts in New Jersey
one school district registers 0 for two years 2003, 2004
among the other 4 - two school districts in 2012, one in 2003 and 2005 each */
